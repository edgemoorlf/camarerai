...
from camarerai.providers.dashscope.services.dashscope_service import DashScopeService
from camarerai.common.services.llm_service import LLMService
...
def process_chat(session_id, transcript):
    ...
    if tool_call_buffer.get('name') == 'update_order':
        ...
        # If LLM didn't provide text response, generate a default confirmation
        if not content_buffer.strip():
            action = result.get('action', 'updated')
            item_names = [item.get('name', 'item') for item in result.get('order', {}).get('items', [])]
            if item_names:
                confirmation = f"I've added {', '.join(item_names)} to your order. Anything else?"
            else:
                confirmation = "I've updated your order. Is there anything else you'd like?"

            # Send default confirmation via TTS
            socketio.emit('synthesis_started', {'session_id': session_id})
            ...
            # Add to content buffer for session history
            content_buffer = confirmation
...
IMPORTANT: After using the update_order function, ALWAYS respond with a friendly conversational message confirming what was ordered. Never stay silent after updating an order.
