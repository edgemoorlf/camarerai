...
Tests for DashScope ASR + LLM + TTS implementation
"""
...
class DashScopeTester:
    """Test runner for DashScope provider"""
    ...
    def run_all(self) -> Dict:
        """Run all tests"""
        print("\n" + "=" * 60)
        print("DASHSCOPE PROVIDER TESTS")
        print("=" * 60)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Test DashScope Provider')
    parser.add_argument('-i', '--iterations', type=int, default=1, help='Iterations per scenario')
    args = parser.parse_args()

    tester = DashScopeTester(iterations=args.iterations)
    results = tester.run_all()

    # Save report
    report_dir = os.path.join(os.path.dirname(__file__), 'reports')
    os.makedirs(report_dir, exist_ok=True)
    report_path = os.path.join(report_dir, f"test_dashscope_{int(time.time())}.json")
